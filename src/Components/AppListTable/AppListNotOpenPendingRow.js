...
function AppListNotOpenPendingRow() {
    const {viewApp,setViewApp} =useContext(viewAppContext)

    return (
  
      viewApp.filter((filter)=>filter.view==false).map((element,index)=>
     ...
     <td className="py-4 px-6 hidden md:block">
      ...
      <td className="py-4 px-6">
          ...
          <button className='hover:bg-orange-500 border-b border bg-transparent hover:border-white p-2  border-black ' onClick={()=>{
             axios.get(`/admin/updateSatus?status=Processing&appId=${element._id}`).then((response)=>{
...
export default AppListNotOpenPendingRow